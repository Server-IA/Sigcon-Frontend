                                ...
                                <Fragment key={module.id}>
                                    ...
                                    </li>
                                    {
                                        ...
                                        module.menus?.map((menu) => {
                                            const activeParent = isParentActive(module, menu, location.pathname);
                                            const hasChildren = menu?.childrens?.length > 0;
                                            
                                            return hasChildren ? (
                                                <MenuChildrenItem key={`${module.id}-${menu.id}`} url_module={module.url} option={menu} active={activeParent} />
                                            ) : ( 
                                                <MenuItem key={`${module.id}-${menu.id}`} url_module={module.url} option={menu} active={activeParent} />
                                            )
