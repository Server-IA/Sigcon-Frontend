...
// }
  ...
  const loadData = async () => {
    try{
      const [
        ...
        fetchHelper.post(base_url(["api", "v1", "banks/checkbooks/search"]),
          { length: -1, columns: [{data: "status", search: {value: "ACTIVA", regex: false}}], },
          {},
          0
        )
      ]);
      ...
      setCheckbooks(checkbooksResponse.data);
  ...
  const calculateRulerRetention = (rulerRetentionId) => {
    ...
    console.log(accountingAccount, "Accounting Account");
    if(accountingAccount){
      if(accountingAccount.pucAccount.code.startsWith("2367")){
        ...
        console.log(rulerTax, "Ruler Tax");
                    ...
                    <InputSelectModal
                      ...
                      onChange={(value) =>
                        setAssets({ ...assets,
                          ...
                          cashAccountId: value == 1 ? null : Number(value),
                          checkId: value == 2 ? null : Number(value),
                          bankAccountId: value == 3 ? null : Number(value)
                        })
                      }
                      options={
                        assets.paymentMethodId == 1 ? cash.map(c => ({ id: c.id, label: `${c.cashCode} - ${c.cashName}` })) :
                        assets.paymentMethodId == 2 ? checkbooks.map(c => ({ id: c.id, label: `${c.checkbookNumber} - ${c.issuingBank}` })) :
